...
public class MarketplaceController {
 ...
 @GetMapping("/api/orders") public Object orders(HttpServletRequest req){User u=auth.user(req);String guest=(String)auth.session(req).getAttribute("guest");return store.read(d->d.orders.values().stream().filter(o->auth.canView(u,guest,o)).map(o->view(o,u)).toList());}
 ...
 @PostMapping("/api/checkout") public synchronized Object create(@RequestBody CheckoutRequest r,HttpServletRequest req)throws StripeException{
  String buyer=auth.buyer(req);checkout.validateToken(r.checkoutToken());String mail=email(r.email()),name=text(r.name(),80,"name"),address=text(r.address(),200,"delivery address");
 ...
 private Object checkoutView(Order o,String secret){Map<String,Object> r=new LinkedHashMap<>();r.put("orderId",o.id);r.put("amount",o.amount);r.put("status",o.paymentStatus);r.put("mode",mode());if(secret!=null)r.put("clientSecret",secret);return r;}
 ...
 @PostMapping("/api/orders/{id}/allocate") public synchronized Object allocate(@PathVariable String id,HttpServletRequest req)throws StripeException{
  ...
  if(!demo()){refreshPartner("restaurant",o.restaurantId);refreshPartner("courier",o.courierId);}
